 ...
 * Additionally, it places all non-related elements at the bottom.
 *
 * @author bobus
 */
class OrganizerNatural extends OrganizerAlphabetical {

    /**
     * Resistance between objects - "push force"
     */
    private final static int RESISTANCE = 1800000;

    /**
     * Gravity between objects - "pull force"
     */
    private final static int GRAVITY = 32;

    /**
     * Force limit per cycle. Required so that close objects do not get sling-shot too far.
     */
    private final static int FORCE_LIMIT = 1500;
    ...
    private void organizeSchema(DraggableComponent schema) {
        ...
        if (schema instanceof Schema) {
            ...
            elems.addAll(scLocal.getRelations().stream()
                    .filter(rel -> rel.getConstraints().stream().anyMatch(con -> con.getRel1() != null && con.getRel2() != null) || !rel.getInheritances().isEmpty())
            ...
            elems.addAll(scLocal.getRelations().stream()
                    .filter(rel -> !rel.getLinks().isEmpty())
                    .collect(Collectors.toList())
            );
        ...
        if (elems.size() <= 1 && schema instanceof Schema) {
            ...
            int leftMost = 999999999;
            Point placeTaken = new Point(0, 0);
            for (DraggableComponent elem : elems) {
                ...
                if (leftMost > elem.getX()) {
                    leftMost = elem.getX();
                }
                if (placeTaken.x < elem.getX() + elem.getWidth()) {
                    placeTaken.x = elem.getX() + elem.getWidth();
                }
                if (placeTaken.y < elem.getY() + elem.getHeight()) {
                    placeTaken.y = elem.getY() + elem.getHeight();
                }
            }
            putElementsAtBottom(schema, placeTaken, leftMost);
        }
        schema.checkSize();
    }

    private void putElementsAtBottom(DraggableComponent schema, Point placeTaken, int leftMost) {
        List<DraggableComponent> elems = new ArrayList<>();
        if (schema instanceof Schema) {
            Schema scLocal = (Schema) schema;
            elems.addAll(scLocal.getRelations().stream()
                    .filter(rel -> rel.getConstraints().stream().noneMatch(con -> con.getRel1() != null && con.getRel2() != null) && rel.getInheritances().isEmpty())
                    .collect(Collectors.toList())
            );
            for (Function func : scLocal.getFunctions()) {
                if (func.hasNoTriggers()) {
        ...
        // Set minimum width by available elements (if only few are available) or 5
        int minElements = elems.size() < 5 ? elems.size() : 5;
        int minElementsTotalW = leftMost + (Canvas.DEFAULT_ENTITY_WIDTH + Canvas.GRID_SIZE) * minElements;
        if (placeTaken.x < minElementsTotalW) {
            placeTaken.x = minElementsTotalW;
        }

        int perWidth = (int) Math.ceil(Math.sqrt(elems.size()));
        int perWidthByTakenPlace = (placeTaken.x - leftMost) / (Canvas.DEFAULT_ENTITY_WIDTH + Canvas.GRID_SIZE);
        if (perWidth < perWidthByTakenPlace) {
            perWidth = perWidthByTakenPlace;
        }

        int left = leftMost;
        int top = placeTaken.y;
        int tallest = 0;

        top = Geometry.getSnappedPosition(0, top + tallest + V_PAD).y;
        for (DraggableComponent elem : elems) {
            elem.setLocation(Geometry.getSnappedPosition(left, top));
            elem.checkConstraints();
            elem.isOrganized = true;

            if (tallest < elem.getSize().height) {
                tallest = elem.getSize().height;
            }
            left += H_PAD + elem.getSize().width;
            if (leftMost + perWidth * (Canvas.DEFAULT_ENTITY_WIDTH + H_PAD) < left + elem.getSize().width) {
                left = leftMost;
                top = Geometry.getSnappedPosition(0, top + tallest + V_PAD).y;
                tallest = 0;
            }
    ...
    private void pull(DraggableComponent local, DraggableComponent remote, Point rLoc) {
        ...
        double force = GRAVITY * Math.pow(dist, 0.6D) / 100D;

        if (
                (local instanceof Relation && remote instanceof Relation &&
    ...
    private void push(DraggableComponent local, DraggableComponent remote) {
        ...
        double force = RESISTANCE / Math.pow(dist, 1.7);
        if (force > FORCE_LIMIT) {
