...
public abstract class SQLEnabledWizard extends MigWizard {
	...
	void drawSuccessWindow(JFrame frame){
		JOptionPane.showMessageDialog(frame, new SelectableText("Query executed successfully", false), "Query successful", JOptionPane.INFORMATION_MESSAGE);
	}
	protected void drawErrorWindow(JFrame frame, String title, String message, String exMessage){
		JOptionPane.showMessageDialog(frame, new SelectableText(message + connection.getCleanError(exMessage), false), title, JOptionPane.ERROR_MESSAGE);
	}
